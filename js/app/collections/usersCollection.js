	...
	var UsersCollection = Backbone.Collection.extend({
		initialize: function(models, options) {
			...
			if (_thisCollection.online==1) {
				// http://s299455960.online.de:5000/users/?id=042cb1572ffbea5d
				if (this.options.id && !this.options.dbid) this.options.dbid = this.options.id;
				if (this.options.dbid) this.url = 'http://s299455960.online.de:5000/users/?id='+this.options.dbid;
				if (this.options.sponsor) this.url = this.url+'&sponsor='+this.options.sponsor;
				// else this.url = 'http://s299455960.online.de:5000/users/';
