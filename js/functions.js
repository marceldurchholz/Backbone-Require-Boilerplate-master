	...
	try {
		...
		ft.upload(mediaFile,
			...
			function(r) {
				...
				console.log("Response = " + r.response);
				console.log("Sent = " + r.bytesSent);
				dpd.videos.post({"uploader":""+_this._thisViewRecordVideoNested.me.id,"videourl":""+options.fileName,"topic":""+options.interest,"title":""+formValues.title,"subtitle":""+formValues.subtitle,"description":""+formValues.description,"price":formValues.sliderprice}, function(result, err) {
					if(err) {
						// $.mobile.loading( 'hide' );
						hideModal();
